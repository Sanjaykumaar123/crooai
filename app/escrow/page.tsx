...
export default function EscrowPage() {
  ...
  return (
    <WalletGate 
      title="Connect Wallet to Audit Escrows" 
      description="Connect your Web3 MetaMask wallet to view, verify, and settle decentralized escrow smart contracts."
    >
                    ...
                    <div className="p-5 glass-card rounded-2xl space-y-6">
                      ...
                      {selectedEscrow.status === 'locked' && (
                        <div className="pt-4 border-t border-neutral-100 space-y-2.5">
                          <button
                            onClick={async () => {
                              const res = await releaseEscrow(selectedEscrow.id);
                              if (res && res.success) {
                                alert(`Transaction confirmed! Hash: ${res.txHash}`);
                              } else {
                          ...
                          <button
                            onClick={async () => {
                              const res = await refundEscrow(selectedEscrow.id);
                              if (res && res.success) {
                                alert(`Refund transaction confirmed! Hash: ${res.txHash}`);
                              } else {
